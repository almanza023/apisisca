...
class BoletinFinalTres extends Model
{
    ...
    public static function reporte($sede, $grado, $pdf)
    {
        ...
        foreach ($matriculas as $matricula) {
            ...
            $matematicas = self::processAreas($sede, $grado, $matricula, $pdf, $ganadas, $perdidas, $matetemicas, $ihsMat, $prefijos, 1, 1.3);
            $promedioGeneral += $matematicas['promedioGeneral'];
            $total += $matematicas['total'];

            // Calificación de Castellano
            $castellano = self::processAreas($sede, $grado, $matricula, $pdf, $ganadas, $perdidas, $matetemicas, $ihsMat, $prefijos, 2, 2.2);
            $promedioGeneral += $castellano['promedioGeneral'];
            $total += $castellano['total'];

             // Calificación de Ciencias Naturales
             $naturales = self::processAreas($sede, $grado, $matricula, $pdf, $ganadas, $perdidas, $matetemicas, $ihsMat, $prefijos, 4, 4.3);
             $promedioGeneral += $naturales['promedioGeneral'];
             $total += $naturales['total'];

             // Calificación de Ciencias Naturales
             $sociales = self::processAreas($sede, $grado, $matricula, $pdf, $ganadas, $perdidas, $matetemicas, $ihsMat, $prefijos, 5, 5.5);
             $promedioGeneral += $sociales['promedioGeneral'];
             $total += $sociales['total'];
    ...
    private static function processAreas($sede, $grado, $matricula, $pdf, &$ganadas, &$perdidas, &$matetemicas, &$ihsMat, $prefijos, $ordernI, $ordenF)
    {
        $calAreas = Calificacion::reportCalificaciones($matricula->id, 4, $ordernI, $ordenF);
        $total = 0;
        $ihs = 0;
        $definitiva = 0;
        foreach ($calAreas as $cal) {
            $total = 1;
        ...
        self::addAreaCalificacionToPDF($pdf, $ihs, $area, $definitiva, $definitiva);
        self::addLogroToPDF($definitiva, $ganadas, $perdidas, $prefijos, $pdf);
        return ['promedioGeneral' => $definitiva, 'total' => $total];
    }

    private static function processOtrasAreas($sede, $grado, $matricula, $pdf, &$promedioGeneral, &$total, &$ganadas, &$perdidas, $prefijos)
    {
        $calificaciones = Calificacion::reportCalificaciones($matricula->id, 4, 5.9, '');
